  ...
  private
    def send_reply
      if self.reply_changed? and !self.reply.blank?
        Twitter.update("@#{self.screen_name} #{self.reply} https://www.foodbornechicago.org/#{self.tweet_id}?utm_source=twitter&utm_medium=tweet&utm_content=#{self.tweet_id}&utm_campaign=foodborne", :in_reply_to_status_id => self.tweet_id)
      end
    end
end
